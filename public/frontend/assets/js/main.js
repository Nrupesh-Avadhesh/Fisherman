(function ($) {
  ...
  //     range: "min",
  //     min: min,
  //     max: max, // Adjust the maximum value as needed
  //     value: min,
  //     behaviour : 'drag',
  //     tooltips : true,
  //     slide: function(event, ui) {
  //         $(".loan-amount").val(ui.value);
  //         var fee = (ui.value * 10) / 100;
  //         $(".payment_tit").text("$ "+ui.value+" USD Pay");
  //         $("#fees").text("$" + fee + " USD");
  //         $("#payamount").val(ui.value);
  //     },
  //   });
  
  $("#loan-range-bar-cus").attr("min",min);
  $("#loan-range-bar-cus").attr("value",min);
  $("#loan-range-bar-cus").attr("max",max);
  $(document).on("input", '#loan-range-bar-cus', function(e) {
      $('.loan-amount').val($(this).val());
      const progress = ($(this).val() / max) * 100;
      const sliderEl = document.querySelector("#loan-range-bar-cus")
      sliderEl.style.background = `linear-gradient(to right, #04AA6D ${progress}%, transparent ${progress}%)`;
      amount_val();
  })
  
      $('.loan-amount').keypress(function(e) {
      ...
      $(document).on("change", '.loan-amount', function(e) {
          amount_val();
      })
      function amount_val(){
          if (!$('.loan-amount').val()) {
              $('.loan-amount').val(min);
          } else if ($('.loan-amount').val() < min) {
              $('.loan-amount').val(min);
          } else if ($('.loan-amount').val() > max) {
              $('.loan-amount').val(max);
          }
          amount_cal(1);
      }
      function amount_cal($n = 0){
          if($n == 0){
              // if (!$('.loan-amount').val()) {
              //     $('.loan-amount').val(min);
              // }
              if ($('.loan-amount').val() > max) {
                  $('.loan-amount').val(max);
              }
          }
          var amo = $('.loan-amount').val();
          var per = (amo * 100)/ max;
          if(amo){
              $('#loan-range-bar-cus').val(amo);
          } else {
              $('#loan-range-bar-cus').val(0);
          }
          const progress = (amo / max) * 100;
          const sliderEl = document.querySelector("#loan-range-bar-cus")
          sliderEl.style.background = `linear-gradient(to right, #04AA6D ${progress}%, transparent ${progress}%)`;
          // $('.ui-loan-range').css('width',per+'%');
